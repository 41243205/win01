...
MainWindow_H::MainWindow_H(QWidget *parent)
    : QMainWindow(parent), tableWidget(new QTableWidget(this)), lives(3), selectedRow(-1), selectedCol(-1)
{
    ...
    connect(tableWidget, &QTableWidget::cellChanged, this, &MainWindow_H::onCellChanged);
    connect(tableWidget, &QTableWidget::cellClicked, this, &MainWindow_H::onCellClicked);
}

MainWindow_H::~MainWindow_H() {}

void MainWindow_H::resizeEvent(QResizeEvent *event) {
...
bool MainWindow_H::solveSudoku(int row, int col) {
...
bool MainWindow_H::isSafe(int row, int col, int num) {
    for (int i = 0; i < 16; ++i)
...
void MainWindow_H::generateSudokuSolution() {
...
void MainWindow_H::setupBorders() {
...
void MainWindow_H::onCellChanged(int row, int column) {
    QTableWidgetItem *item = tableWidget->item(row, column);
    if (!item || row != selectedRow || column != selectedCol) return;

    disconnect(tableWidget, &QTableWidget::cellChanged, this, &MainWindow_H::onCellChanged);
...
void MainWindow_H::onCellClicked(int row, int column) {
    QTableWidgetItem *item = tableWidget->item(row, column);
...
void MainWindow_H::checkWinCondition() {
    for (int row = 0; row < 16; ++row) {
        for (int col = 0; col < 16; ++col) {
            QTableWidgetItem *item = tableWidget->item(row, col);
...
void MainWindow_H::checkGameOver() {
    if (lives <= 0) {
